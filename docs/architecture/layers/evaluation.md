...
- **Status:** Provisional — realised by Stage 3 (rolling-origin splitter, shipped), Stage 4 (metrics + fold-level harness + three-way NESO benchmark, shipped), and Stage 6 (diagnostic-plot helper library + fixed-window NESO benchmark bar chart, shipped). Revisit at Stage 9 (registry integration — which may force evaluation to quote a registry entry by ID) and Stage 18 (drift monitoring placement).
...
- **Concrete instances:** [Stage 3 retro](../../lld/stages/03-feature-assembler.md) (the splitter); [Stage 4 retro](../../lld/stages/04-linear-baseline.md) (metrics, harness, benchmarks); [Stage 6 retro](../../lld/stages/06-enhanced-evaluation.md) (diagnostic-plot helper library).
