...
| `models/scipy_parametric.py` | SciPy `curve_fit` parametric load | 8 | Shipped | Hand-specified `α + β_heat · HDD + β_cool · CDD + diurnal + weekly Fourier` form with Elexon-convention fixed hinges (plan D1: `T_heat = 15.5 °C`, `T_cool = 22.0 °C`); 13 parameters default (plan D2: `diurnal_harmonics=3` + `weekly_harmonics=2`). Default solver is `method="trf"` with physically-motivated bounds (plan 08a D1/D2: `alpha ∈ [0, 100 000]`, `beta_heat`/`beta_cool ∈ [0, 5 000]`, Fourier coefficients `∈ [−50 000, 50 000]`); Gaussian-CI reasoning still holds for the default `loss="linear"` provided no parameter is at a bound (plan D3). Three `pcov`-unreliability overrides surface unidentifiable parameters as `std_err = inf`: bound saturation (parameter within ~`1e-6 · width` of either bound), implausibly large Moore-Penrose variance (diagonal entry > `((upper − lower) / 2)²`), zero-information Jacobian column (design-matrix row std < `1e-12`).  `pcov` row and column for each unreliable parameter is set to `np.inf` end-to-end so the saved covariance matrix stays self-consistent. `p0` derived deterministically from training data inside `fit()` (plan D4). Covariance matrix round-trips via `ModelMetadata.hyperparameters["covariance_matrix"]` as `list[list[float]]` (plan D7). `_parametric_fn`, `_derive_p0`, `_build_param_names` are module-level pure functions (plan S2). `ScipyParametricConfig.feature_columns` constrains *Fourier* columns, not raw inputs (plan D2 clarification). Serialised via `save_skops` (Stage 12 D10 T2). |
