...
**Status:** `approved` — Ctrl+G on 2026-04-24 accepted the decision table with three amendments (D1 → CUDA-aware torch install, D7' → re-scope seeding to cover the CUDA / MPS paths, D11 → auto-select device across CUDA / MPS / CPU). H-5 / OQ-A (use_deterministic_algorithms) resolved in the negative per the D7' re-scope. **D1 subsequently trimmed mid-T2 on 2026-04-24:** the Dockerfile pre-warm clause is dropped — the persistent `.venv` mount on `/workspace` means the first-run torch install cost is a one-time hit per container image, not per session, and reaching outside the Docker sandbox during image build was adding risk without commensurate benefit. `pyproject.toml`'s cu128 index pin is retained; the first `uv sync` inside a fresh container pulls the CUDA wheel, and subsequent sessions reuse the existing `.venv`. Ready for Phase 2.
...
|---|---|---|---|---|
| **D1** | Runtime dependency | **Add `torch>=2.7,<3` to `[project].dependencies`, resolved from the CUDA 12.8 wheel index on Linux and from PyPI elsewhere.** Configured via `[tool.uv.sources]` + a `[[tool.uv.index]] name = "pytorch-cu128"` entry pointing at `https://download.pytorch.org/whl/cu128`; `torch` is pinned to that index under `marker = "sys_platform == 'linux'"` and to PyPI otherwise. The PyPI wheel for `torch` ships MPS support on Apple Silicon out of the box, so no second index is needed for macOS. The Dockerfile already targets `nvidia/cuda:12.8.0-cudnn-runtime-ubuntu24.04` (Blackwell / RTX 5090, sm_120), and cu128 is the first PyTorch wheel track with stable Blackwell support — the torch lower bound is bumped from the research-draft's `>=2.3` to `>=2.7` accordingly. **No Dockerfile change**: the persistent `.venv` mount on `/workspace` means a one-time `uv sync` per container image brings torch into the venv and subsequent sessions reuse it. An earlier draft of this decision proposed a `docker build` pre-warm step via `uv pip download`; it is **dropped on 2026-04-24 mid-T2** because it reaches outside the Docker sandbox during image build without commensurate benefit once the venv persists. The cu128 index stays in `pyproject.toml` so the one-time install picks the Blackwell-compatible wheel automatically; no manual intervention is required inside the container. | Intent §Scope names "a small MLP" and DESIGN §8 names PyTorch as the chosen framework; no MLP = no stage. AC-5 says "no GPU requirement", not "no GPU" — training the meetup model on a laptop still works with the PyPI wheel, and on the RTX 5090 dev host the CUDA wheel is many orders of magnitude faster, which materially widens the range of experiments a facilitator can run live. The persistent venv makes a Dockerfile pre-warm redundant for the workstation dev loop; CI can cache `.venv` or `.uv-cache` directly if the first-install cost matters. | Intent AC-1 / AC-5; DESIGN §8; Dockerfile L12 (cu128 / Blackwell). Domain research §R3 (serialisation under PyTorch 2.6+ `weights_only` default; `state_dict` contract stable across 2.7). |
...
- `Dockerfile` — **no change** (D1 dropped the Dockerfile pre-warm on 2026-04-24; the persistent `.venv` makes it unnecessary, and it reached outside the Docker sandbox during image build).
...
- `Dockerfile` — **not touched** (D1 dropped the pre-warm clause mid-T2 on 2026-04-24).
