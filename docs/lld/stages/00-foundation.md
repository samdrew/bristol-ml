...
- `docs/architecture/README.md` (pointer into the project spec §3), ADRs 0001 and 0002 under `docs/architecture/decisions/`, this retrospective under `docs/lld/stages/`.
...
- **`conf/__init__.py`**: added so `conf._schemas` is importable. Not listed in the project spec §6's original tree; §6 updated in this same PR.
...
tree -L 2                                # matches docs/intent/DESIGN.md §6
```

## Deferred

- The `Model` protocol (`src/bristol_ml/models/base.py`) — arrives at Stage 4 when the first model needs it. The pattern is committed here in ADR 0001 and the project spec §7.3.
- Logging framework (`loguru` per the project spec §8) — no module has anything to log yet; `print` is deliberate at Stage 0.
...
- MkDocs site (the project spec §8) — defer until `docs/` has enough content to render.
